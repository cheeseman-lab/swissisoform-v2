"""Dormant module: protein-similarity "conservation" via DIAMOND/blastp/MMseqs2.

Kept for reference after the conservation rewrite (2026-04-21) that moved the
canonical conservation signal to Zoonomia BigWig lookups
(see ``conservation.py`` and ``docs/reviews/conservation_module_spec.md``).
Not wired into the pipeline.  Preserved so protein-homology evidence can be
reintroduced as a distinct module later without re-deriving it.
"""
...
class ConservationHomologyModule:
