...
The Predicted-Structure read used to see only what ``evidence.slice_category``
could pre-compress: 13 scalars from ``P1_structured_extension`` and 13 from
``P2_shared_structural_change`` — pLDDT means/std, pTM, three PAE block
aggregates, RMSD, TM-score. Behind them sit a per-residue confidence array and an
L×L predicted-aligned-error matrix. From the scalars the model can report
"diff-region pLDDT 0.62" but cannot ask whether that is a uniformly floppy tail
or a confident helix followed by a disordered linker, nor whether an extension is
docked against one specific part of the core or merely "placed" on average.

These readers move that compression from precompute time to query time. They are
exposed as Anthropic tools (:data:`P_TOOLS`), executed locally by the dispatcher
from :func:`make_p_dispatch`, and driven by
``swissisoform.site.llm.run_tool_loop``.

**P is the inverse of M.** The Mutation-Landscape slice was bulky (30 raw hit
rows, 93% of the payload) with thin access to the real table. The P slice is the
smallest of the six categories and carries no hit rows at all, so there is
nothing to strip — all the bulk is behind these readers. That moves the entire
risk to what a reader RETURNS, and the governing rule is complexity in L:

* **O(L) may return raw** — a whole per-residue pLDDT array is ~1.4k chars.
* **O(L²) must aggregate** — a full PAE matrix is ~332k chars at L=239, four
  times the entire pre-strip M slice, and the opening context plus every tool
  result is re-sent on each turn. :func:`pae_block` therefore returns
  mean/min/max over the requested rows × cols and never the sub-matrix; the model
  gets its resolution by choosing the ranges.
* **Bounded-in-practice still caps** — :func:`contacts` caps its partner list.

Addressing: the fold cache is keyed by ``sha1`` of the protein sequence, and
``all_paired.parquet`` carries only protein *lengths*. ``StructureModule`` emits
``structure_{canonical,isoform}_hash`` for exactly this reason, and
``build_gene_record`` mirrors every parquet column into ``_raw``, so the readers
resolve a ``tis_id`` to a cache entry with no new artifact. See
``docs/architecture/structure_subsystem.md``.

Everything here is a pure read. No file is written; the only persisted state is
an in-process cache of parsed structures.
"""
