...
model.cif), which is unusable for a human. This maps each isoform (and its
...
def export_structures(genes, outdir: Path) -> tuple[int, int, int, Path]:
    """Copy cached Boltz .cif files for *genes* into *outdir* + write a manifest.

    Returns ``(n_iso, n_can, n_missing, manifest_path)``.
    """
