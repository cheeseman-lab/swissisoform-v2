...
class TestExtractWindowMinus:
    ...
    def test_noncanonical_codon_strand_aware(self):
        # minus-strand transcript whose start codon is CTG (not ATG): the window
        # must open on the called codon — the assumption the runtime
        # start-codon guard in resolve.py checks.
        mrna = "ACGTACGTAC" + "CTG" + "C" * 17
        assert mrna[10:13] == "CTG"
        genome = FakeGenome({"chr1": "N" * 10 + _rc(mrna) + "N" * 20})
        coords = _coords("-", [(10, 40)])
        assert build_transcript_mrna(coords, genome) == mrna
        win = extract_tis_window(coords, genome, start_genomic=30, up=5, down=6)
        assert win.downstream[:3] == "CTG"
