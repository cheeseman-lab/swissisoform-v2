"""Precompute per-residue folding colours for the interactive 3Dmol viewer.
...
def main(argv: list[str] | None = None) -> int:
    ...
    structures_dir = args.structures.resolve()
    colors_dir = (args.colors or structures_dir / "colors").resolve()
    ...
    for _, row in df.iterrows():
        ...
        for side, key in (("canonical", (gene, "canonical")), ("isoform", (gene, seg))):
            ...
            if not colors:
                ...
                continue
            (colors_dir / out_name).write_text(json.dumps(colors, separators=(",", ":")))
            written += 1
